...
import 'package:aviatickets_testapp/features/all_tickets_page/data/data_source/remote_data_source/all_tickets_remote_data_source.dart';
import 'package:aviatickets_testapp/features/all_tickets_page/data/model/all_tickets_model.dart';
import 'package:aviatickets_testapp/features/all_tickets_page/data/model/extension/extension_all_tickets_model_to_entity.dart';
...
@LazySingleton(as: FetchAllTicketsRepository)
class FetchAllTicketsRepositoryImpl implements FetchAllTicketsRepository{
  FetchAllTicketsRepositoryImpl(this.networkInfo, this.allTicketsRemoteDataSourceClient);
  final NetworkInfo networkInfo;
  final AllTicketsRemoteDataSourceClient allTicketsRemoteDataSourceClient;

  @override
  Future<Either<Failure, AllTicketsEntity>> fetchAllTickets() async{
    if(! await networkInfo.isConnected) return Left(ServerFailure());
    try{
      //final model = await allTicketsRemoteDataSourceClient.client().fetchAllTickets();
      //final entity = model.toEntity();
      // TODO вэб-сервис с моками не работает поэтому так
      final modelMock = AllTicketsModel.fromJson(myMap);
      final entityMock = modelMock.toEntity();

      return Right(entityMock);
